...
public class JwkAuthenticationManagerResolver implements
    ...
    private String toIssuerId(HttpServletRequest request) {
        ...
        try {
            if (StringUtils.hasText((String) JWTParser.parse(token).getJWTClaimsSet().getClaim("preferred_username"))) {
                return "sso";
            }else if (StringUtils.hasText((String) JWTParser.parse(token).getJWTClaimsSet().getSubject())) {
                return "internal";
            } else {
